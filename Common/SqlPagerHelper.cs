...
namespace ZGZY.Common
{
    ...
    public class SqlPagerHelper
    {
        ...
        /// <param name="where">查询条件</param>
        /// <param name="totalCount">总记录数</param>
        public static DataTable GetPagerNew(string tableName, string columns, string order, int pageSize, int pageIndex, string where,string columns2, out int totalCount)
        {
            SqlParameter[] paras = { 
                                   new SqlParameter("@tablename",SqlDbType.VarChar,100),
                                   new SqlParameter("@columns",SqlDbType.VarChar,500),
                                   ...
                                   new SqlParameter("@pageIndex",SqlDbType.Int),
                                   new SqlParameter("@where",SqlDbType.VarChar,2000),
                                   new SqlParameter("@columns2",SqlDbType.VarChar,500),
                                   new SqlParameter("@totalCount",SqlDbType.Int)
                                   };
            paras[0].Value = tableName;
            paras[1].Value = columns;
            ...
            paras[4].Value = pageIndex;
            paras[5].Value = where;
            paras[6].Value = columns2;
            paras[7].Direction = ParameterDirection.Output;   //输出参数

            DataTable dt = ZGZY.Common.SqlHelper.GetDataTable(ZGZY.Common.SqlHelper.connStr, CommandType.StoredProcedure, "sp_Pager_New", paras);
